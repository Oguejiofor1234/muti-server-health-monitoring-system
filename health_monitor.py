...
from config import SERVERS, THRESHOLDS, EMAIL_CONFIG, MONITORING_CONFIG, LOG_FILE, ALERT_LOG_FILE, SLACK_CONFIG
from slack_notifier import SlackNotifier


class ServerMonitor:
    """Monitors a single server via SSH"""
    ...
    def execute_command(self, command):
        ...
        try:
            ...
            if exit_code == 0:
                ...
                print(f"Command failed on {self.name}: {error}")
                return None
    ...
    def get_metrics(self):
        ...
        try:
            ...
            print(f"✗ Error collecting metrics from {self.name}: {e}")
...
class AlertManager:
    ...
    def send_email_alert(server_name, metrics, alerts):
        ...
        if SLACK_CONFIG['enabled']:
            try:
                slack = SlackNotifier(SLACK_CONFIG['webhook_url'])
                slack.send_alert(server_name, metrics, alerts)
                print(f"✓ Slack alert sent for {server_name}")
            except Exception as e:
                print(f"✗ Slack notification failed: {e}")
        
        # Send email if configured (or if Slack disabled)
        if not SLACK_CONFIG['enabled'] or SLACK_CONFIG.get('send_email_also', True):
            try:
                # Create message
                msg = MIMEMultipart()
    ...
    def log_metrics(metrics):
        ...
        try:
            ...
            with open(LOG_FILE, 'a', newline='') as f:
                fieldnames = ['timestamp', 'server_name', 'status', 'cpu_percent', 
                             'memory_percent', 'disk_percent', 'uptime']
